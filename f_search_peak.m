function [peak_idx, peak_val] = f_search_peak(fft_spectrum, search_length, threshold, max_target_count, Min_Distance, Max_Distance, DistPerBin)
    ...
    % go to the 3rd sample and compare with 2 samples before and after the
    % current 3rd sample
    for n = 3:(search_length - 3)
        ...
        % checks if current sample magnitude is above the threshold we set
        % and higher than its 2 sample before/after neighbour
        if(fp >= threshold && fp >= fl2 && fp >= fl && fp > fr && fp > fr2)
            
            % calculate the current bin range based on we defined earlier
            curr_range = (fp_bin - 1) * DistPerBin;
            fprintf("===FFT sample: %d, Range: %.2f ===\n", fp_bin, curr_range); 

            
            % checks if above our min max distance we set
            if (curr_range >= Min_Distance && curr_range <= Max_Distance)
                
                % increment count
                peak_cnt = peak_cnt + 1;
                
                % set our current peak value of magnitude at our current sample index
